...
void modify_ptr_via_copy(std::shared_ptr<Point> point) {
  point->SetX(20);
  std::cout << "Use count of shared pointer is " << point.use_count() << std::endl;
}
...
int main() {
  ...
  // s3 are not empty.
  std::cout << "Pointer s1 is " << (s1 ? "not empty" : "empty") << std::endl;  // empty
  std::cout << "Pointer s2 is " << (s2 ? "not empty" : "empty") << std::endl;  // not empty
  std::cout << "Pointer s3 is " << (s3 ? "not empty" : "empty") << std::endl;  // not empty
  ...
  // use_count which keeps track of the number of objects  currently interacting with the same internal data as the
  // current shared pointer instance

  // Then, s4 is copy-constructed from s3.
  // This is copy-construction because it is the first time s4 appears.
  // Note: we cannot do it with unique_ptr
  ...
  std::cout << "Number of shared pointer object instances using the data in s3 "
               "after one copy: "
            << s3.use_count() << std::endl;  // 2

  // Then, s5 is copy-constructed from s4.
  std::shared_ptr<Point> s5(s4);
  // above create a new pointer s5, this increases the reference count, this will have another owner for the object
  // Memory Management: The Point object will be destroyed only when all std::shared_ptr instances that manage it are
  // destroyed or reset.
  // Can be used for multithreading, it will be safe because the object will be destoryed after the count is 0, but by
  // reference the count will not change

  // below create an alias to s5, this does not increase the reference count, it will not call the
  // copy constructor
  // std::shared_ptr<Point> &s6 = s5;
  // generally not meaningful, because we can just pass s5's reference directly

  // Now, the number of references to pointer s3's data should be 3, since s5,
  // s4, and s3 have access to s3's data.
  std::cout << "Number of shared pointer object instances using the data in s3 "
               "after two copies: "
            << s3.use_count() << std::endl;  // 3

  // Modifying s3's data should also change the data in s4 and s5, since they
  // refer to the same object instance.
  s3->SetX(445);

  std::cout << "Printing x in s3: " << s3->GetX() << std::endl;  // all changed to 445
  ...
  // instance data, not 4.
  std::cout << "Pointer s5 is " << (s5 ? "not empty" : "empty") << std::endl;  // empty
  std::cout << "Number of shared pointer object instances using the data in s3 "
               "after two copies and a move: "
            << s3.use_count() << std::endl;  // still 3
  ...
  // reference.
  modify_ptr_via_ref(s2);   // pass by reference, set x to 15
  modify_ptr_via_copy(s2);  // pass by value, set x to 20
  // by ref and by copy does the same functionality, the count number and memory management are different
  modify_ptr_via_rvalue_ref(std::move(s2));  // pass by rvalue reference, set y to 645
  // since the function does not move the rvalue to a new lvalue, the original s2 is still usable after the function
  ...
  // to 1.
  std::cout << "Number of shared pointer object instances using the data in s2: " << s2.use_count()
            << std::endl;  // count: 1
  // so here it does not pass by reference, it passes by value, so the count will increase (similar to we create a copy
  // and pass it to the function)
  copy_shared_ptr_in_function(s2);  // count: 2
  std::cout << "Number of shared pointer object instances using the data in s2 "
               "after calling copy_shared_ptr_in_function: "
            << s2.use_count() << std::endl;  // back to 1
