...
class Solution {
   public:
    vector<vector<int>> permute(vector<int>& nums) {
        // Second Idea: Use currentPermutation array instead of index
        vector<vector<int>> result;
        vector<bool> used(nums.size(), false);
        vector<int> currentPermutation;
        // it starts with an empty currentPermutation
   ...
   private:
    void backtrack(vector<int>& nums, vector<vector<int>>& result, vector<int>& currentPermutation,
        ...
        for (int i = 0; i < nums.size(); ++i) {
            // for each i, we will check if it is used, if it is used, we will skip it
            // here it means the i-th position element is already in the Permutation
            // this prevents from [1,1,1] cases
            if (used[i]) continue;
            used[i] = true;
